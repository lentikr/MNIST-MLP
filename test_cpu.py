...
optimizer = torch.optim.Adam(net.parameters(), lr=LEARNING_RATE)      # 定义优化器，这里采用 torch.optim 库
...
for epoch in range(NUM_EPOCHS):
    with tqdm(train_loader, unit='batch') as train_epoch:
        ...
        for images, labels in train_epoch:      # 调取训练数据
            ...
            i = i + 1
            train_epoch.set_postfix(loss=(train_loss/i).data.numpy())
            time.sleep(0.0001)

        torch.save(net.state_dict(), os.path.join('model/MLP_epoch_{}.pth'.format(epoch))) #存储训练得到的参数
    ...
    if EVAL_TEST:
        with tqdm(test_loader, unit='batch') as test_epoch:
            ...
            for images, labels in test_epoch:
                ...
                correct = correct + float((pred_y == labels.data.numpy()).astype(int).sum())
